...
export default async function HomePage() {
  const [metrics, recent, settings] = await Promise.all([
    getDashboardMetrics(undefined, { publicOnly: true }),
    getReportList({}, { publicOnly: true }),
    getCommunitySettings(),
  ]);
  const trackableTotal = metrics.total - metrics.rejected;
  const handledCount = metrics.acknowledged + metrics.working + metrics.resolved;
  const handledPercent = trackableTotal ? Math.round((handledCount / trackableTotal) * 100) : 0;
  const statusRows = [
    { label: "รอตรวจสอบ", value: metrics.pending, color: "bg-amber-500" },
    { label: "รับเรื่องแล้ว", value: metrics.acknowledged, color: "bg-sky-500" },
    { label: "กำลังดำเนินการ", value: metrics.working, color: "bg-indigo-500" },
    { label: "แก้ไข/ปิดเรื่องแล้ว", value: metrics.resolved, color: "bg-emerald-500" },
  ];
  const stats: { label: string; value: string | number; icon: Icon }[] = [
    { label: "ปัญหาที่แจ้งทั้งหมด", value: metrics.total, icon: IconReport },
    { label: "รับเรื่อง/ดำเนินการ", value: metrics.inProgress, icon: IconProgress },
  ...
  return <><SiteHeader /><main>
    ...
    <section className="border-b bg-muted/20"><div className="mx-auto grid max-w-7xl gap-6 px-4 py-12 sm:px-6 lg:grid-cols-[.8fr_1.2fr] lg:px-8"><div className="self-center"><p className="text-sm font-semibold text-primary">โปร่งใส ตรวจสอบได้</p><h2 className="mt-1 font-heading text-3xl font-bold tracking-tight">สถานะการรับเรื่องของหน่วยงาน</h2><p className="mt-3 max-w-xl leading-7 text-muted-foreground">ข้อมูลจากรายงานสาธารณะ อัปเดตตามการบันทึกของเจ้าหน้าที่ เพื่อให้เห็นว่ารับเรื่องแล้วหรือกำลังดำเนินการถึงขั้นตอนไหน</p></div><Card><CardHeader><CardTitle className="flex items-center justify-between gap-3"><span>ภาพรวมการดำเนินงาน</span><span className="text-2xl text-primary">{handledPercent}%</span></CardTitle></CardHeader><CardContent className="space-y-6"><Progress value={handledPercent}><ProgressLabel>รายงานที่หน่วยงานรับดำเนินการแล้ว</ProgressLabel><ProgressValue /></Progress><div className="grid gap-3 sm:grid-cols-2">{statusRows.map((item) => <div key={item.label} className="flex items-center gap-3 rounded-2xl bg-muted/50 p-4"><span className={`size-2.5 rounded-full ${item.color}`} /><span className="flex-1 text-sm text-muted-foreground">{item.label}</span><span className="text-lg font-bold tabular-nums">{item.value}</span></div>)}</div>{metrics.rejected > 0 && <p className="text-xs text-muted-foreground">ไม่รับเรื่อง {metrics.rejected} รายงาน โดยแสดงเหตุผลไว้ในไทม์ไลน์ของแต่ละรายงาน</p>}</CardContent></Card></div></section>
